...
//  Templated operator overloads added by me ( Gálffy Tamás ) because I felt like it

#include <cstddef>   // std::size_t
#include <cstring>   // std::memcpy, std::memcmp, std::memset, std::memchr, std::strlen
...
template <typename T>
buffer& operator<<(buffer& b, const T& x)
{
	static_assert(std::is_pod<T>::value, "Only POD values");
	b.append(&x, sizeof(T));
	return b;
}
